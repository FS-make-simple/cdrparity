...
cat $SDRR $SDRP $SDRV $SDRR $SDRP $SDRV >test_00.tmp
...
$SDRP1 -b 1k -s "$low_size"k -p test_02.tmp
echo
if ! $SDRV test_02.tmp; then
    echo 'FAILED!'
    exit
fi
echo
if ! $SDRR test_02.tmp test_03.tmp; then
    echo 'FAILED!'
    exit
fi
...
$SDRP1 -b 1k -s "$high_size"k -p test_04.tmp
echo
if ! $SDRV test_04.tmp; then
    echo 'FAILED!'
    exit
fi
echo
if ! $SDRR test_04.tmp test_05.tmp; then
    echo 'FAILED!'
    exit
fi
